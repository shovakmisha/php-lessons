			...
			<div class='content col-lg-10 col-md-9 col-sm-9 col-xs-12'>
				...
				<?php 
					if (isset($name) && isset($link)){
						$result = mysql_query ("UPDATE friends SET name='$name', link='$link' WHERE id='$id'");
						if ($result == 'true') {echo "<p class='message'>Информация о друге успешно обновлена!</p>";}
						else {echo "<p class='message'>Информация о друге не обновлена!</p>";}
					}else{
						echo "<p class='message'>Вы ввели не всю информацию, поэтому информация о друге не может быть обновлена.</p>";
					}
				?>
			</div>
		</div>
