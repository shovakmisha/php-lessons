...
function & XMLRPC_prepare($data, $type = NULL){
	if(is_array($data) || is_object($data)){  // Иван Никитин: для PHP5 проверка is_object обязательна!
...
function XMLRPC_request($site, $location, $methodName, $params = NULL, $user_agent = NULL){
	...
	if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
		XMLRPC_debug('XMLRPC_request', "Received the following parameter list to send:" . XMLRPC_show($params, 'print_r', true));
	}
...
function XMLRPC_response($return_value, $server = NULL){
	...
	echo $return;
	//file_put_contents('xmlrpc.xml', $return);
}
...
function XMLRPC_show($data, $func = "print_r", $return_str = false){
	...
	ob_end_clean();
	// Иван Никитин: просто отдадим строку
	return $output . "\n";
	/*
	if($return_str){
