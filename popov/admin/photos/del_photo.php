	...
	<div class='container'>
		...
		<div class='row'>
			...
			<div class='content col-lg-10 col-md-9 col-sm-9 col-xs-12'>
				...
				<form class='forma' action='drop_photo.php' method='post'> 
					<p class="podskazka">Bыберите картинку, которую вы хотите удалить</p>
					<?php 
						$result = mysql_query("SELECT title,mini,id FROM fotos");      
						$myrow = mysql_fetch_array($result);
						do{
							printf ("<div>
										<label>
											<input name='id' type='radio' value='%s'> <img src='../../%s' style='width: 30px; margin: 10px;' > %s
										</label>
									</div>"
										,$myrow["id"],$myrow["mini"],$myrow["title"]);
